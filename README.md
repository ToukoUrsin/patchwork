...
The campaign owner must create the real account/project and provision its token. This source does not contain a fabricated project ID, credentials or claimed successful integration run.
...
3. Finnish and Spanish become stale because English changed. Save each against the current English revision. The app does not pretend a machine presence check validates translation meaning.
...
Mutations use `@sanity/client`, one transaction, and exact `ifRevisionID` preconditions for every previously read app document. Newly created operation receipts use `create`, not overwrite. A stale source or concurrent workflow write returns a conflict. A lost transaction response triggers a receipt read before any retry; identical operation ids are idempotent only when the entire command matches.
...
Tests cover translation lineage, fresh approvals, source invalidation, immutable published snapshots, restoration, role guards, stale-version conflicts, idempotency, full document mapping, atomic transaction preconditions, lost-success reconciliation and seed non-overwrite. Cloud adapter tests use an explicitly labeled transaction test double; they are **not evidence of a live Sanity run**. Current validation and remaining checks: [VALIDATION.md](docs/VALIDATION.md).
